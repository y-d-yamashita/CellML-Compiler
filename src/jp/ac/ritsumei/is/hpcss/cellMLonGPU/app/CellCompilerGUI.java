...
public class CellCompilerGUI  extends JFrame {
	...
	private static final int dialogWidth = 750;
	private static final int dialogHeight = 650;
	...
	/*生成系1*/
	private JRadioButton gJavaButton;
	private JRadioButton gCButton;
	...
	public static void main(String[] args) {
		new CellCompilerGUI().setVisible(true);
	}
	
	/**
	 * インスタンスを作成し数式展開システムGUI版を実行する.
	 */
	public CellCompilerGUI() {
		super("CellCompiler2012");
		...
		// RelML ラベル
		recmlLabel = new JLabel("Input File");
		...
		recmlButton = new JButton("Browse...");
		...
		// 生成器1
		gCButton = new JRadioButton("C program      ");
		gJavaButton = new JRadioButton("Java program     ");
		...
		// プログラムファイル ラベル
		programLabel = new JLabel("Output File Name");
		...
		selectPanel0.setBorder(new TitledBorder(new EtchedBorder(), "Program Type"));
		selectPanel0.add(gJavaButton);
		selectPanel0.add(gCButton);
		...
		selectPanel.setBorder(new TitledBorder(new EtchedBorder(), "Input File Type"));
    ...
    private void exec() {
		...
		File xml = new File(getRecML());
    	if (!xml.exists()) {
    		JOptionPane.showMessageDialog(this,
    				"ファイルが存在しません: " + getRecML(),
    				"エラー", JOptionPane.ERROR_MESSAGE);
    		return;
    	}
    	...
    	if (gJavaButton.isSelected()) {
    		
    		genCode0 = CellCompilerMain.GENERATOR_JAVA;
    		
    		
    	}else if(gCButton.isSelected()){
    		
    		genCode0 = CellCompilerMain.GENERATOR_C;
    	}

    	/*生成系2*/
    	String genCode1 = null;
    	if (rSimpleButton.isSelected()) {
    		genCode1 = CellCompilerMain.FILETYPE_SIMPLE;
    	}else if(rStructuredButton.isSelected()){
    		genCode1 = CellCompilerMain.FILETYPE_STRUCTURED;    		
    	}else if(rRelmlButton.isSelected()){
    		genCode1 = CellCompilerMain.FILETYPE_RELML;
    	}
