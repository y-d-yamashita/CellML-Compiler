...
public class ODECommonProgramGeneratorSimulMain {
	...
	//	終了コード		: int
	//===================================================
	public static void main(String[] args) throws MathException, CellMLException, RelMLException, TranslateException, SyntaxException, TableException {
...
//		String xml = args[2];
		...
		if(args[0].equals(GENERATOR_KAWABATA)){
			if(args[1].equals(FILETYPE_SIMPLE)){
				...
				SimpleRecMLVariableTable vartable = simpleRecMLAnalyzer.getRecMLVariableTable();
				PairList<RecMLVertex, RecMLVertex> pairList = simpleRecMLAnalyzer.resultMaximumMatching;
				...
				for(int i=0;i<pairList.size();i++){
					...
					//導出変数を取得
					SimpleRecMLVariableReference derivedTable = vartable.getVariableReference(varID);
...
//				/*selector内cnのInteger*/
				...
				try {
					pProgramGenerator =
					new ODECommonProgramGeneratorSimul(ｒecMLAnalyzer);
...
//				/*selector内cnのInteger*/
				...
				try {
					pProgramGenerator =
					new ODECommonProgramGenerator(pRecMLAnalyzer);
					} catch (MathException e1) {
					// TODO 自動生成された catch ブロック
					e1.printStackTrace();
				}
