...
import jp.ac.ritsumei.is.hpcss.cellMLonGPU.graph.manipulator.algorithm.LazyMaximumMatching;
import jp.ac.ritsumei.is.hpcss.cellMLonGPU.graph.manipulator.algorithm.LoopCreator;
import jp.ac.ritsumei.is.hpcss.cellMLonGPU.graph.manipulator.algorithm.LoopSeparator;
import jp.ac.ritsumei.is.hpcss.cellMLonGPU.graph.manipulator.algorithm.MaximumMatching;
...
public class GraphManipulator {
	...
	private LazyMaximumMatching lazyMaximumMatching;
	...
	public GraphManipulator(){
		...
		lazyMaximumMatching = new LazyMaximumMatching();
		tarjan = new Tarjan();
	...
	public FieldGraph cretateFieldDependencyGraph(
			DirectedGraph<RecMLVertex, RecMLEdge> graph,
			SimpleRecMLAnalyzer recmlAnalyzer
			) throws GraphException, TableException, MathException{
			return graphCreator.cretateFieldDependencyGraph(graph,recmlAnalyzer.getRecMLVariableTable(),recmlAnalyzer);
	}
	...
	/**
	 * Maximum matching
	 * @param graph, simpleRecMLAnalyzer, (option) useExpressionIdFlag
	 * @return List of pairs<RecMLVertex,RecMLVertex> 
	 * @throws GraphException
	 * @see Pair
	 * @see PairList
	 */
	public PairList<RecMLVertex,RecMLVertex> lazyMaximumMatching(
			BipartiteGraph<RecMLVertex,RecMLEdge> graph,
			SimpleRecMLAnalyzer simpleRecMLAnalyzer,
			boolean useExpressionIdFlag
			) 
			throws GraphException{
		return lazyMaximumMatching.run(graph,simpleRecMLAnalyzer,useExpressionIdFlag);
	}
	
	
	public PairList<RecMLVertex,RecMLVertex> lazilyMaximumMatching(
			BipartiteGraph<RecMLVertex,RecMLEdge> graph,
			SimpleRecMLAnalyzer simpleRecMLAnalyzer) 
			throws GraphException{
		return lazyMaximumMatching.run(graph,simpleRecMLAnalyzer,true);
	}
	...
	public List2D<RecMLVertex> compressDependencyList(List2D<RecMLVertex> inputList,DirectedGraph<RecMLVertex, RecMLEdge> graph){
		List2D<RecMLVertex> outputList = new List2D<RecMLVertex>();
		for(List<RecMLVertex> scc:inputList){ ///
		     for(RecMLVertex node:scc){  /// Check all element in reverse SccList
		     int orderIndex=outputList.size();//Set disenable index value
		    LABEL: for(int index=outputList.size()-1;index>=0;index--){
		     for(RecMLVertex orderNode:outputList.get(index)){
		     for(RecMLVertex v:graph.getOutOfVertex(orderNode)){ // if exit edge that orderNode is src and node is dest
		     if(v==node){ // (orderList)node ---> (orig)node
		     break LABEL;
		     }
		     }
		     }
		     ...
		     if(orderIndex==outputList.size()){ // node is tail
		    List<RecMLVertex> newScc = new ArrayList<RecMLVertex>();
		    newScc.add(node);
		    outputList.add(newScc);
		     }else{
		     outputList.get(orderIndex).add(node);
		     }
		     }
		     }
		 return outputList;    
