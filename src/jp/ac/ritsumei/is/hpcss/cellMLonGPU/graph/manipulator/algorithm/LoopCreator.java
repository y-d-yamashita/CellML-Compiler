...
public class LoopCreator {
	...
	public MathExpressionLoop create(FieldVertexGroupList groupList) throws MathException{
		MathExpressionLoop loopList=converteLoopStructure(groupList);
	...
	private MathExpressionLoop converteLoopStructure(FieldVertexGroupList groupList) {
		...
		if(groupList.getXAxisLoopGroupList()!=null){
			for(List<FieldVertexGroup> timeLevel:groupList.getXAxisLoopGroupList()){
				...
				for(FieldVertexGroup xAxisLevel:timeLevel){
					...
					if(xAxisLevel.getVertexGroup().size()>0){
	...
	private MathExpressionLoop merge(MathExpressionLoop loopList,FieldVertexGroupList groupList) throws MathException{
		...
		//ToDo:
		/*
		 *Sorting based on Calculation order and Axis[t][x][y][z]
		 */
		
		//Timeループ化
		if(groupList.getTimeLoopGroupList()!=null){
			for(int i=1;i<loopList.getMathExpressionLoopList().size();i++){
				if(loopList.getLoop(i-1).isPossibleMerge(loopList.getLoop(i),timeIndexPosition)){
					loopList.getLoop(i-1).merge(loopList.getLoop(i),timeIndexVariable,timeIndexPosition);
					loopList.getMathExpressionLoopList().remove(i);
					i--;							
				}
			}	
		}
