...
public class HighSpeedDecisionLoopStructureTest {
	...
	public static void main(String[] args) {
		...
		pattern = new RelationPath(1,3,"post");
		inputList.add(pattern);
		pattern = new RelationPath(1,4,"post");
		inputList.add(pattern);	
		pattern = new RelationPath(2,3,"post");
		inputList.add(pattern);
		pattern = new RelationPath(2,4,"post");
		inputList.add(pattern);
		pattern = new RelationPath(3,1,"pre");
		inputList.add(pattern);
		pattern = new RelationPath(3,2,"pre");
		inputList.add(pattern);
		pattern = new RelationPath(3,4,"inner");
		inputList.add(pattern);
		pattern = new RelationPath(4,1,"pre");
		inputList.add(pattern);
		pattern = new RelationPath(4,2,"pre");
		inputList.add(pattern);
		...
		for(int i=0;i<loop_nameListPair.size();i++){
			...
			if(fixed_inputList!=null &&check_inh(fixed_inputList,inputList)){
				...
				for(int j=0;j<loop_nameListPair.get(i).size();j++){
					for(int k=0;k<loop_nameListPair.get(i).get(j).length;k++){
						System.out.print(loop_nameListPair.get(i).get(j)[k]);
					}System.out.println();
				}
				*/
				break;
