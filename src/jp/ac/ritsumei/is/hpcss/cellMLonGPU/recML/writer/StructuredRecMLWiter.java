...
public class StructuredRecMLWiter{
	...
	public static void main(String[] args) throws MathException, GraphException {
...
//		System.out.println("make StructuredRecML Output Start.");
		ArrayList<String> out = new ArrayList<String>();

		out.add("<recml>");
		out.add("\n");
		
//		System.out.println("make StructuredRecML LoopIndex Output Start.");
		ArrayList<String> li = this.outPutLoopIndex();
		out.addAll(li);
		out.add("\n");

//		System.out.println("make StructuredRecML LoopStructure Output Start.");
		ArrayList<String> ls = this.outPutLoopStructureV2();
		out.addAll(ls);
		out.add("\n");

//		System.out.println("make StructuredRecML Variable Output Start.");
		ArrayList<String> var = this.outPutVariable();
		out.addAll(var);
		out.add("\n");

//		System.out.println("make StructuredRecML DepGraph Output Start.");
		ArrayList<String> dgr = this.outPutDepGraph();
		out.addAll(dgr);
		out.add("\n");
		
//		System.out.println("make StructuredRecML SimulEqu Output Start.");		
		ArrayList<String> se = this.outPutSimulEqu();
		out.addAll(se);
		out.add("\n");

//		System.out.println("make StructuredRecML RefVariableList Output Start.");		
		ArrayList<String> rvl = this.outPutRefVariableList();
		out.addAll(rvl);
		out.add("\n");

//		System.out.println("make StructuredRecML MathML Output Start.");		
		out.add("<math>");
...
//		System.out.println("make StructuredRecML Output Finih.");
	}
	...
	public ArrayList<String> outPutLoopStructureV2() {
		ArrayList<String> out = new ArrayList<String>();

		ArrayList<RelationPattern> rp = pSimpleRecMLAnalyzer.getM_LoopStrucuture();
		ArrayList<Integer> pareNameList = new ArrayList<Integer>();
		
		for(RelationPattern soro_rp:rp){
			pareNameList.add(soro_rp.Parent_name);
		}
		for(RelationPattern soro_rp:rp){
			pareNameList.remove(soro_rp.Child_name);
		}

		Integer rootNum = pareNameList.get(0);
		int tab = 0;
		String str = outPutLoopStructureV2_1(rootNum,tab);
		out.add(str);
...
//			str = "<variable name=\"" + key.toLegalString() + "\" type=\"constvar\"/>";
			str = "<variable name=\"" + key.toLegalString() + "\" type=\"constvar\" "+"initial_value=\""+key.getValue()+"\" ";
			if(loopCompo.containsKey(key)){
				HashMap<Integer,Integer> hm = loopCompo.get(key);
				for(Integer i:hm.keySet()){
					if(hm.get(i) == 1){
