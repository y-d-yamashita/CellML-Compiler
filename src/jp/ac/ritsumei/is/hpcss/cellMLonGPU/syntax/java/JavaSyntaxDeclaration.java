 ...
 * @author n-washio
 */
public class JavaSyntaxDeclaration extends JavaSyntax {
	...
	/**内部の構文*/
	protected JavaSyntaxDataType m_pSynDataType;
	 ...
	 */
	public JavaSyntaxDeclaration(JavaSyntaxDataType pSynDataType, MathOperand pMathOperand) {
		super(eSyntaxClassification.SYN_JAVA_DECLARATION);
	...
	throws MathException, SyntaxException {
		...
		/*宣言文追加*/
		strPresentText.append(m_pSynDataType.toLegalJavaString() + " "
				+ m_pMathOperand.toLegalJavaString() + m_pSynDataType.toStringSuffix());
		...
		if (m_pInitExpression != null) {
			...
			if (pInstance.getMode() &&
					m_pSynDataType.toLegalJavaString().equals("BigDecimal")) {
				strPresentText.append(" = new BigDecimal( "
						+ m_pInitExpression.toLegalJavaString()+ ")");
			} else {
	...
	throws MathException, SyntaxException {
		return m_pSynDataType.toLegalJavaString() + " "
			+ m_pMathOperand.toLegalJavaString() + m_pSynDataType.toStringSuffix();
	}
	 ...
	 * @return 一致判定
	 */
	public boolean matches(JavaSyntaxDeclaration pDeclaration) {
		/*宣言変数の照合結果を返す*/
